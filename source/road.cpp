...
void Road::initialize_periodic(float density, float car_ratio, float p_lambda){
    ...
    /* Initializes Cars */
    if (car_ratio > 0){
		place_vehicle_type((*make<car>), number_car, p_lambda);
    }
    /* Initializes Motorcycles */
    if (motor_ratio > 0){
        number_motorcycle = number_vehicles - vehicle_array.size();
		place_vehicle_type((*make<motorcycle>), number_motorcycle, p_lambda);
    }
    /* Cleanup Operations */
...
void Road::place_vehicle_type(vehicle* (*veh_type)(), int number, float p_lambda){
	int pos, lane, counter;
	short width = veh_type()->width, length = veh_type()->length;
	vector<int> lane_choice(lanes / veh_type()->width);
